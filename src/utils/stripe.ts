...
export const isValidStripeCustomer = async (
  customerId: string | null | undefined
) => {
  return await stripe.customers
    .retrieve(customerId ?? "placeholder")
    .then((customer) => {
    ...
    .catch((e) => {
      const error = e as Stripe.errors.StripeAPIError;
      if (error.code === "resource_missing" && error.statusCode === 400) {
        return undefined;
      }
    });
};
